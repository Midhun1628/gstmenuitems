    ...
    >
      <Slide v-for="(slide, i) in slideShow" :key="i">
        <div class="carousel__item w-100" aria-hidden="true">
          ...
          <v-btn variant="text" icon class="wishlistIcon" rounded="md" aria-hidden="true" @click="toggleWishlist(slide.id)">
            <SvgSprite name="custom-heart-fill" class="text-error" v-if="isInWishlist(slide.id)" style="width: 18px; height: 18px" />
            <SvgSprite name="custom-heart-outline" v-else class="text-lightText" style="width: 18px; height: 18px" />
          </v-btn>
        </div>
      </Slide>
    </Carousel>
